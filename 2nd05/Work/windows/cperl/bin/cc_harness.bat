...
sub cc_harness_msvc {
    ...
    if ($debug) {
      ...
      $compile .= " -Od" if $ENV{APPVEYOR} and $^O eq 'MSWin32';
      if ($ENV{PERL_CORE} and $Config{ccflags} =~ /-flto/) { # too big
        $compile =~ s/-O\d / -O1 /;
      }
    }
...
my $ccflags = $Config{optimize}." ".$Config{ccflags};
# crashes on cygwin
...
if ($debug and $Config{cc} =~ /\bgcc/ and $Config{gccversion}) {
...
$ccflags .= " --no-warn"
  if $Config{cc} =~ /\bgcc/ and $Config{gccversion} and $quiet and $^O ne 'darwin';
