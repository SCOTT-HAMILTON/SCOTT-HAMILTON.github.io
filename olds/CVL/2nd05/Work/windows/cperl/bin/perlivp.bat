...
my $perlpath = 'c:\cperl\bin\perl.exe';
...
$label = 'Modules needed for rest of cperlivp exist';
...
if ($extensions_total == $extensions_there) {
    ...
    print "# extensions_total: $extensions_total\n";
    print "# extensions_there: $extensions_there\n";
    $error_total++;
}
$tests_total++;
...
for (@modules) {
    ...
    # or ignored
    @missing = grep { ! m|^none/.*\.[13](pm)?| } @missing;
...
if (($installed_total == $installed_there) && 
    ...
    print "# installed_total: $installed_total\n";
    print "# installed_there: $installed_there\n";
...
# Final report (rather than feed ourselves to Test::Harness::runtests()
...
cperlivp - Perl Installation Verification Procedure

=head1 SYNOPSIS

B<cperlivp> [B<-p>] [B<-v>] [B<-h>]

=head1 DESCRIPTION

The B<cperlivp> program is set up at Perl source code build time to test the
...
One of the two modules that is used by cperlivp was not present in the 
installation.  This is a serious error since it adversely affects cperlivp's
