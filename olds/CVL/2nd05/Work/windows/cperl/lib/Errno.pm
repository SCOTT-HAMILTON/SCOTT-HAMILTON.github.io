...
"MSWin32-x64-multi-thread-6.3.9600" or
	die "Errno architecture (MSWin32-x64-multi-thread-6.3.9600) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEPROTOTYPE WSAEREMOTE WSAEDISCON WSAEINTR WSAEADDRNOTAVAIL
	WSAEMSGSIZE WSAEINVALIDPROVIDER WSAEPROCLIM WSAENOTEMPTY
	WSAEWOULDBLOCK WSAEINPROGRESS WSAEHOSTDOWN WSAESHUTDOWN WSAELOOP
	WSAEALREADY WSAEACCES WSAEINVALIDPROCTABLE WSAEDQUOT WSAENETUNREACH
	WSAENOPROTOOPT WSAEPROVIDERFAILEDINIT WSAEDESTADDRREQ WSAENOTSOCK
	WSAEHOSTUNREACH WSAEOPNOTSUPP WSAEINVAL WSAESOCKTNOSUPPORT
	WSAECONNABORTED WSAENETDOWN WSAETOOMANYREFS WSAECONNRESET WSAENETRESET
	WSAEBADF WSAEFAULT WSAEAFNOSUPPORT WSAECONNREFUSED WSAENOMORE
	WSAECANCELLED WSAEADDRINUSE WSAEISCONN WSAEPROTONOSUPPORT
	WSAENAMETOOLONG WSAESTALE WSAEUSERS WSAENOTCONN WSAETIMEDOUT
	WSAEREFUSED WSAEPFNOSUPPORT WSAENOBUFS WSAEMFILE
    )],
);
